...
// Fetch existing data
const { data: item } = await useFetch(`/api/admin/social-highlights/${itemId}`)
...
async function submit() {
  loading.value = true
  try {
    await $fetch(`/api/admin/social-highlights/${itemId}`, {
      method: 'PUT',
      body: form.value
    })
...
<template>
  <div class="max-w-2xl mx-auto px-4 md:px-0">
    <div class="flex items-center gap-4 mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-900">Edit Highlight</h1>
    </div>

    <div v-if="!item" class="text-center py-12">Loading...</div>

    <div v-else class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 md:p-6">
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div>
          ...
          <input v-model="form.title" type="text" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
        </div>
        ...
        <div class="pt-4 flex items-center justify-end gap-3 border-t border-gray-100">
          ...
          <button type="submit" :disabled="loading" class="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition disabled:opacity-50">
            {{ loading ? 'Saving...' : 'Update Highlight' }}
          </button>
        </div>
      </form>
    </div>
