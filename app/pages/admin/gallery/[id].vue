...
onMounted(async () => {
  try {
    // Ideally we'd valid single fetch here, but sticking to existing pattern for now
    // or filtering from the list if strict single endpoint missing.
    // Let's rely on the current method to avoid breaking backend assumptons
    const all = await $fetch('/api/admin/gallery') as any[]
    const found = all.find((i: any) => i.id === route.params.id)
    
    if (found) {
        form.value = {
            ...found,
            // Convert array tags to comma-string for input
            tags: Array.isArray(found.tags) ? found.tags.join(', ') : (found.tags || ''),
            published_at: found.published_at ? found.published_at.split('T')[0] : ''
        }
    } else {
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border overflow-y-auto">
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea v-model="form.description" rows="3" class="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition"></textarea>
                </div>
                ...
                <div>
                    ...
                    <input v-model="form.src" class="w-full border rounded-lg p-2.5 font-mono text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition" />
                </div>

                <div v-if="form.type === 'video'">
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Poster URL (Cover Image)</label>
                    <input v-model="form.poster" class="w-full border rounded-lg p-2.5 font-mono text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition" />
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Tags</label>
                    <input v-model="form.tags" class="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition" />
